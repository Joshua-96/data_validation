...
class Validator:
    ...
    def _handle_None(self, instance, value):
    ...
    def __set__(self, instance: object, value):
        ...
        value = self._handle_None(instance, value)
        ...
        if not isinstance(value, self.annotated_type)\
                and value is not None\
                and not isinstance(value, dict):
            # pass test if int is compared to float
            try:
                if isinstance(sub_annotated_type, type(None)):
                    fct_to_call = TypeHandler.TYPE_MAPPING[
                        (self.sub_value_type, sub_annotated_type)]
                    value = map(fct_to_call, value)
                else:
                    fct_to_call = TypeHandler.TYPE_MAPPING[
                        (self.value_type, self.annotated_type)]
